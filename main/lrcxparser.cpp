...
int LrcLine::highlightCount(qint64 lineOffsetMs) const {
    ...
    for (int i = 0; i < chars.size() - 1; ++i) {
        const CharTiming &cur =
            chars[i];
        const CharTiming &next =
            chars[i + 1];
        qint64 startTime =
            cur.offsetMs;
        qint64 endTime =
        ...
        int endIndex =
            next.charIndex;
        if (lineOffsetMs >= startTime &&
            lineOffsetMs < endTime) {
            int charCount =
                endIndex - startIndex;
            if (charCount <= 0)
                return startIndex;
            double t =
                double(lineOffsetMs - startTime)
...
qint64 LrcxParser::parseTimestamp(
    const QString &inner) {
    ...
    while (ms3.size() < 3)
        ms3 += '0';
    qint64 ms =
        ms3.left(3).toLongLong();
    return mins * 60000
           + secs * 1000
           + ms;
}
void LrcxParser::parseTt(
    const QString &body,
    LrcLine &line) {
    static const QRegularExpression re(
        R"(<([^>]+)>)"
...
bool LrcxParser::load(
    ...
    clear();
    static const QRegularExpression tsTagRe(
        R"(\[(\d+:\d+\.\d+)\])"
        );
    static const QRegularExpression ttTagRe(
        R"(\[tt\])"
        );
    while (!in.atEnd()) {
        QString raw =
        ...
        QRegularExpressionMatchIterator mit =
            tsTagRe.globalMatch(raw);
        QList<QRegularExpressionMatch> matches;
        while (mit.hasNext())
            matches.append(mit.next());
        if (matches.isEmpty())
            continue;
        qint64 startTs =
            parseTimestamp(
                matches.first().captured(1)
                );
        if (startTs < 0)
            continue;
        timestamps.append(startTs);
        int textStart =
            matches.first().capturedEnd();
        int textEnd =
            raw.length();
...
int LrcxParser::currentLine(
    ...
    for (int i = 0;
         i < m_lines.size();
         ++i) {
        if (m_lines[i].startMs <= posMs)
            res = i; else
            break;
    }
    return res;
}
QString LrcxParser::findLyricFile(
    const QString &audioPath,
    const QString &lyricsDir) {
    QFileInfo afi(audioPath);
    QString base =
        afi.fileName();
    QString stem =
        afi.completeBaseName();
    QStringList dirs;
